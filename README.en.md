...
After an upgrade removes an old versioned cache, already-open tasks may still retain the old MCP process. Starting in 0.4.8, a process whose pinned `grb.py` disappeared selects the highest complete installed plugin runtime under the same marketplace/plugin cache root and includes a `runtime_handoff` receipt in the result. Version 0.4.9 preserves the `jobs` shape for `grok_status` and rejects leftover stubs or shims. A new task is still needed to refresh Skill instructions and MCP schemas, but old tasks no longer lose job management solely because their version directory was replaced.
