...
Read-only structured review modes also own a bounded transport-recovery policy. The first attempt is assigned a stable Grok session. After an explicit `reqwest` stream/send/decode failure, up to two recovery attempts resume that session with tools disabled and request only the schema-conforming final result within the original job deadline. `attempt-N.stdout`, `attempt-N.stderr`, and `attempt_details` preserve each try, strategy, and recovery timeout. The session remains discoverable by `continue` after terminal failure. Modes that can write or trigger external side effects receive no implicit retry. When all review attempts fail, `error_kind=transport` remains primary and `contract_error` stays null because no valid model response reached schema validation; the terminal result is not caller-retryable after the companion's recovery budget is exhausted.
